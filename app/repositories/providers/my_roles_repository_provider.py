...
class MyRolesRepositoryProvider(
    ...
    def __init__(self, db: AsyncSession):
        super().__init__(db=db, entity_class=UserRolesEntity)

    async def exists_by_user_id_and_role_id(self, user_id: int, role_id: int) -> bool:
        stmt = select(func.count(UserRolesEntity.id)).where(
            and_(
                UserRolesEntity.role_id == role_id,
                UserRolesEntity.user_id == user_id
            )
        )
    ...
    async def get_by_user_id_and_role_id(self, user_id: int, role_id: int) -> (UserRolesEntity | None):
        stmt = select(UserRolesEntity).where(
            and_(
                UserRolesEntity.role_id == role_id,
                UserRolesEntity.user_id == user_id
            )
