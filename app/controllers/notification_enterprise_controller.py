...
async def toggle_is_view_end(
    noti_id: int,
    notification_service: NotificationEnterpriseServiceProvider = Depends(get_notification_enterprise_provider_dependency),
    ...
    try:
        ...
        noti_updated: Final = await notification_service.toggle_is_view(noti)

        out = noti_updated.to_out()

        return ORJSONResponse(
            status_code=status.HTTP_200_OK,
            content=dict(ResponseBody[dict](
                message="Notification status updated with successfully",
...
@router.get(
    '',
    status_code=status.HTTP_200_OK,
    response_model=Page[NotificationEnterpriseOUT],
)
async def get_all(
    filter: NotificationEnterpriseFilter = Depends(),
    notification_service: NotificationEnterpriseServiceProvider = Depends(get_notification_enterprise_provider_dependency),
    jwt_service: JwtServiceBase = Depends(get_jwt_service_dependency),
    credentials: HTTPAuthorizationCredentials = Depends(bearer_scheme),
):
    try:
        ...
        _all: Final[list[NotificationEnterpriseEntity]] = await notification_service.get_all(filter)
